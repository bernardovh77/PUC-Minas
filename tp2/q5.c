...
int main() {
    ...
    int maior = 0;
    for (int i = 0; i < m; i++) {  // descobre o maior numero de cilindros
        if (selecionados[i].cilindros > maior) {
            maior = selecionados[i].cilindros;
        }
    }

    int count[maior + 1];  // array de contagem
    Veiculo* ordenado = malloc(m * sizeof(Veiculo));

    for (int i = 0; i <= maior; i++) {  // zera as contagens
        count[i] = 0;
    }
    for (int i = 0; i < m; i++) {  // conta quantas vezes cada valor aparece
        count[selecionados[i].cilindros]++;
    }
    for (int i = 1; i <= maior; i++) {  // acumula as contagens
        count[i] += count[i - 1];
    }
    for (int i = m - 1; i >= 0; i--) {  // coloca cada veiculo na sua posicao final
        ordenado[count[selecionados[i].cilindros] - 1] = selecionados[i];
        count[selecionados[i].cilindros]--;
    }

    for (int i = 0; i < m; i++) {  // printa os veiculos ordenados
        char buffer[300];
        formatVeiculo(ordenado[i], buffer);
        printf("%s\n", buffer);
    }
