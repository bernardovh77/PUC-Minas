...
class q7 {
    ...
    public static void main(String[] args) {
        ...
        while (scanner.hasNextInt()) { // le a entrada
        ...
        Veiculo[][] baldes = new Veiculo[10][n]; // cria os 10 baldes
        int[] tamanhos = new int[10];

        for (int i = 0; i < n; i++) { // separa os veiculos nos baldes pela cilindrada / 8.1
            int b = (int) (selecionados[i].getCilindrada() / 8.1 * 10);
            if (b > 9) {
                b = 9;
            }
            baldes[b][tamanhos[b]] = selecionados[i];
            tamanhos[b]++;
        }

        int k = 0;
        for (int b = 0; b < 10; b++) { // ordena cada balde por inserção

            for (int i = 1; i < tamanhos[b]; i++) {
                Veiculo tmp = baldes[b][i];
                int j = i - 1;
                while (j >= 0 && baldes[b][j].getCilindrada() > tmp.getCilindrada()) { // empurra as cilindradas maiores para a direita
            ...
            for (int i = 0; i < tamanhos[b]; i++) { // junta os baldes de volta no array
                selecionados[k] = baldes[b][i];
                k++;
            }
        }
