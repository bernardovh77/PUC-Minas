...
int main() {
    ...
    Veiculo* selecionados = malloc(n * sizeof(Veiculo));  // array com os veiculos da entrada
    int m = 0;
    ...
    int maior = 0;
    for (int i = 0; i < m; i++) {  // descobre o maior ano
        if (selecionados[i].ano > maior) {
            maior = selecionados[i].ano;
        }
    }

    Veiculo* ordenado = malloc(m * sizeof(Veiculo));

    for (int exp = 1; maior / exp > 0; exp *= 10) {  // ordena pelo ano digito por digito, comecando pela unidade
        int count[10];
        for (int i = 0; i < 10; i++) {  // zera as contagens
            count[i] = 0;
        }
        for (int i = 0; i < m; i++) {  // conta os digitos
            count[(selecionados[i].ano / exp) % 10]++;
        }
        for (int i = 1; i < 10; i++) {  // acumula as contagens
            count[i] += count[i - 1];
        }
        for (int i = m - 1; i >= 0; i--) {  // coloca cada veiculo no lugar certo pelo digito
            ordenado[count[(selecionados[i].ano / exp) % 10] - 1] = selecionados[i];
            count[(selecionados[i].ano / exp) % 10]--;
        }
        for (int i = 0; i < m; i++) {  // copia de volta para o array
            selecionados[i] = ordenado[i];
        }
    }

    for (int i = 0; i < m; i++) {  // printa os veiculos ordenados
        char buffer[300];
