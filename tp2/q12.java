...
class Celula { // classe da celula da pilha
...
class Pilha { // classe da pilha com alocação flexivel

    private Celula topo; // variaveis

    public Pilha() {
        topo = null;
    }

    public void inserir(Veiculo veiculo) { // empilha no topo
        Celula tmp = new Celula(veiculo);
        tmp.prox = topo;
        topo = tmp;
    }

    public Veiculo remover() { // desempilha do topo
        Veiculo resp = topo.elemento;
        topo = topo.prox;
        return resp;
    }

    public void mostrar() { // printa os veiculos a partir do topo
...
class q12 {
    ...
    public static void main(String[] args) {
        ...
        while (scanner.hasNextInt()) { // empilha os veiculos da primeira parte
        ...
        for (int k = 0; k < qtd; k++) {
            ...
            if (comando.equals("I")) { // empilha o veiculo do id
                int id = scanner.nextInt();
                for (int i = 0; i < veiculos.length; i++) {
                    if (id == veiculos[i].getId()) {
                        pilha.inserir(veiculos[i]);
        ...
        pilha.mostrar(); // mostra como a pilha ficou
